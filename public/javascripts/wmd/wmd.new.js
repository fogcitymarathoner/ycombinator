	...
	wmd.previewManager = function(){
		...
		var getDocScrollTop = function(){
			...
			return result;
                ...
                var doubleLineBreaks = function(txt) {
                    var result = '';

                    for(var i=0; i<txt.length; i++) {
                        var current = txt.substring(i, i+1);
                        if(current!='\n') {
                            result += current;
                            continue;
                        }
                        var prev = i>0 ? txt.substring(i-1, i) : '';
                        var next = i<txt.length-1 ? txt.substring(i+1, i+2) : '';
                        if(prev == '\n' || next == '\n') {
                            result += current;
                            continue;
                        }
